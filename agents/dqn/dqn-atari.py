...
# from environments.custom_wrapper_atari import GaussianNoiseWrapper
...
class Args:
    ...
    gaussian_background: int = 0
    """adding gaussian noise background"""


def make_env(env_id, seed, idx, capture_video, run_subname):
    def thunk():
        if capture_video and idx == 0:
            env = gym.make(env_id, render_mode="rgb_array")
            # env.metadata["render_fps"] = ?
            env = gym.wrappers.RecordVideo(env, f"videos/{env_id}{run_subname}")
        else:
            env = gym.make(env_id)
        ...
        # Adding Gaussian Noisy Background
        # if args.gaussian_background:
        #     env = GaussianNoiseWrapper(env, mean=0.0, std=args.gaussian_background)
...
"""
    ...
    run_subname = f"{args.exp_name}__{args.seed}__{int(time.time())}"
    run_name = f"{args.env_id}__{run_subname}"
    if args.track:
        import wandb
    ...
    writer = SummaryWriter(f"runs/{args.env_id}/{run_subname}")
    writer.add_text(
        "hyperparameters",
    ...
    envs = gym.vector.SyncVectorEnv(
        [make_env(args.env_id, args.seed + i, i, args.capture_video, run_subname) for i in range(args.num_envs)]
    )
