...
class BisimAgent(object):
    ...
    def update_encoder(self, obs, action, reward, L, step):
        ...
        h2 = h[perm] # permutation of the critic output encoded.
        ...
        r_dist = F.smooth_l1_loss(reward, reward2, reduction='none') # | R_{s_{i}}^{\pi} - R_{s_{j}}^{\pi} |
        if self.transition_model_type == '':
            ...
            transition_dist = torch.sqrt(
                ...
                (    - pred_next_latent_sigma2).pow(2) # (Assumption of Gaussian Distribution) Closed form of the Gaussian distributions for Wasserstein-2 distance of transitions.  Frobenius Norm ??? 
            )
            # transition_dist  = F.smooth_l1_loss(pred_next_latent_mu1, pred_next_latent_mu2, reduction='none') \
