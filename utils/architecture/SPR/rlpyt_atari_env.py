...
class AtariEnv(Env):
    ...
    def generate_hash_seed(self, value):
        # Use a consistent hashing method (e.g., SHA-256)
        # and convert the resulting hash to an integer.
        hash_object = hashlib.sha256(str(value).encode())
        hash_int = int(hash_object.hexdigest(), 16)  # Convert hex to int
        # Keep the seed below 2**31 to avoid overflow issues.
    ...
    def seed(self, seed=None, id=0):
        ...
        # 2**31.
        seed2 = self.generate_hash_seed(seed1 + 1)
    ...
    def reset(self):
        ...
        if self._max_start_noops > 0:
            # Using integers to replace randint for generating random numbers
